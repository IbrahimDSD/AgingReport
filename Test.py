...
def calculate_aging_reports(transactions):
    ...
    # تجميع البيانات بنفس الـ reference مع بعض
    grouped_txs = transactions.groupby(['reference', 'currencyid']).agg({
        'date': 'first',
        'converted': 'sum',
        'vat_amount': 'sum',
        'is_priority': 'first',
    ...
    for _, r in grouped_txs.iterrows():
        entry = {
            'date': r['date'], 
        ...
        if r['currencyid'] == 1:
            (cash_debits if r['converted'] > 0 else cash_credits).append(entry)
        else:
            (gold_debits if r['converted'] > 0 else gold_credits).append(entry)
