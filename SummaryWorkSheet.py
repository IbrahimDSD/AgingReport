...
class CustomPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(50, 10, f"User: {self.username}", 0, 0, 'L')
        self.cell(0, 10, f"Generated on: {self.execution_datetime} | {self.page_no()}/{{nb}}", 0, 0, 'R')


def create_pdf_with_arabic_support(df, grouped=False, username="UnKown",execution_datetime=None):
    execution_time = datetime.now() + timedelta(hours=3)
    execution_datetime = execution_time.strftime('%d/%m/%Y %H:%M:%S')
    usernames = st.session_state.get('username', 'Unknown User')
        ...
        with col_prep2:
            if st.button("🔄 Prepare PDF File", use_container_width=True, help="Generate PDF file for download"):
                with st.spinner("Generating PDF file..."):
                    current_user =st.session_state.get('username', 'Unknown User')  # Replace with actual username if available
